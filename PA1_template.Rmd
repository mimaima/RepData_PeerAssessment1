...
activityData$intervalNum <- (activityData$interval %/% 100) * 12 + (activityData$interval %% 100) / 5 + 1
#interval is transformed to HH:MM time formatted String now
activityData$interval <- format(strptime(sprintf("%04d", activityData$interval), format = "%H%M"), format="%H:%M")
#datetime combines date and time
activityData$datetime <- strptime(paste(activityData$date, activityData$interval, sep=" "), "%Y-%m-%d %H:%M")
...
plot(intervalAverage$intervalNum, intervalAverage$steps, type = "l", main = "Average steps of the 5-minute intervals", xaxt = "n")
...
intervalAverageByDayType <- aggregate( steps~dayType+intervalNum, newActivityData, mean )
intervalAverageByDayType$dayType.f<-factor(intervalAverageByDayType$dayType,levels=c(1,2),labels=c("weekday","weekend")) 
```
...
```{r}
library(lattice)
xyplot(intervalAverageByDayType$steps ~ intervalAverageByDayType$intervalNum | intervalAverageByDayType$dayType.f,
       type = "l",
